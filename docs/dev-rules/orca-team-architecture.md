...
9. **结束 team 必须选择性失效该 team 的自动消息（状态：不变量）**<br>
   每条新 Orca 队列消息必须在 `origin.teamId` 携带 workflow 归属。`end_team` / 关闭协同在进程内 terminal fence 升起后，必须先等待所有在 fence 前取得的 ingress settlement 收敛，再把当前同 team、已落库但仍处于 pre-vendor 的**精确消息项**写入队列快照作为 cleanup intent；随后把 team 持久化为终态，最后才只删除同一 `teamId` 的 pending、pre-vendor active 与 recovery 项，普通用户输入和 scheduler 队列不得受影响。直发路径必须把精确消息项保留到 provider 报告本地 transport 已提交，或在同一个跨进程 dispatch lease 事务内把确认未提交的精确 user row 持久化 rewind；仅成功取得 lease 不足以解除 cleanup tracking，`Session.onDispatching` 更早于该 lease，也不能作为边界。确认未提交时，lease 内只提交 tombstone；media cleanup 与 UI broadcast 在事务提交后完成。cleanup intent 只能记录已知的 clientId/item，不能通过扫描所有历史 user rows 推断；若进程崩在 terminal commit 与失效之间，重启必须从该 intent 恢复并 rewind 精确 user row。若 terminal commit 失败，则必须在重新开放 fence 前结清已准备的 intent，避免带 cleanup 标记的消息重新进入 vendor。崩溃快照恢复时同样以 DB active team 为真源过滤，旧快照缺少 `origin.teamId` 时只允许从历史 `createOpts.vendorOptions.orcaWorkflowId` 兼容读取，仍无法确定归属则 fail-closed；生命周期查询异常属于未知态，必须中止整次恢复并保留原快照供下次重试，不能按 inactive 持久删除。重复 active team 的 read-time dedup 必须在提交 cancelled 终态前采集完整 stale worker/session 清理范围，使查询失败保持数据库未推进，提交后无需再查询才能执行 reconciliation；事务只取消该预读并 fenced 的明确 team IDs，不得用宽谓词误伤并发新建的 team。dormant worker 必须在 resume 前取得 team settlement reservation，并保持到 host dispatch 完成，使 `end_team` 能等待 rehydrate 而不会关闭后又被晚到的 resume 复活。共享 userData / DB 的多个实例还必须以 main 进程持有、不会透明重启的专用 SQLite 连接串行化最终派发与终态写入：派发执行 `BEGIN IMMEDIATE` 后在事务内复核 DB active 状态，只把 writer lease 保持到请求进入 provider 的本地 SDK／进程 transport；provider 启动、网络等待和 RPC response 均不得包含在临界区内。任何版本的结束 UPDATE 都必须竞争同一个 SQLite writer lock，因此不能在 active 复核与本地 vendor 提交之间提交终态；main 进程退出会同时释放连接并终止本地派发。只读进程内 fence 不能替代这个跨实例边界。其他实例无法收到当前进程的内存失效时，queue-head／active-turn recovery 在阻塞新 drain 前还必须现查共享 DB 生命周期；确认 team 已终态时选择性清掉旧 recovery 并继续队列，查询异常则保留 recovery 等下次重试。已持久化但尚未进入 vendor 的 Orca active turn 被终止时，必须 await rewind 对应 user row，避免 transcript 留下未实际派发的幽灵通信。所有异步 queue preparation / restore 完成后、同步 enqueue 前必须再次检查进程内 fence；进入 vendor 前也必须复查。fence 的 DB `pending` 状态只能暂阻派发并保留可重试 recovery，只有 durable `terminal` 状态可永久丢弃消息；提交后的 team-scoped invalidation 会清掉 recovery，DB 回滚则允许它继续重试。尚未形成 queue recovery 的 team-scoped ingress 无论在 reservation 前、同步 enqueue 前、最终 pre-vendor fence，还是离线 session 恢复派发时遇到 `pending`，都必须先结清并释放当前尝试，再等待 transition settle：回滚为 `open` 后重新取得 reservation 再派发，提交为 `terminal` 才确定性拒绝，不能把可能回滚的窗口当成永久消息丢失。实现指针：`agent-input-coordinator.ts` 的 `persistOrcaCleanupIntentWhere` / `discardQueuedItemsWhere` / `isQueueSnapshotItemCurrent` / recovery drain recheck，`makerSendTransaction.ts` 的 pre-rehydrate active-team 检查，`orcaInterAgentDispatcher.ts` 的 team-scoped ingress，`orcaTeamDispatchLease.ts` 的跨实例 lease，以及 `register.ts` 的 `disableOrcaInternal`。

   Worker service 已持有外层 ingress reservation 时，dispatcher 遇到 pending terminal 必须把重试决定交回 service；单次派发的 finally 先释放外层 reservation，再等待 terminal transition。若回滚为 open，重新读取 worker/team、取得 reservation 并恢复 session，避免 terminal 等 ingress、ingress 又等 terminal 的循环等待。`interrupt_worker` 的队首预留和 stop 回调必须在最终 team fence 校验后发生；孤儿初始化的 failed 收口也必须调用 `markTeamEndedWithCleanup`，保留 Lead 的清理范围。
